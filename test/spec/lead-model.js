define([
  ...
  '../../app/scripts/models/lead-model'

], function(LeadModel) {
  'use strict';

  describe("LeadModel", function() {
    var leadModel = new LeadModel({});
    it("should have correct defaults", function() {
      expect(leadModel.get('complete')).to.equal(false);
      expect(leadModel.get('selections')).to.be.an('array');
      expect(leadModel.get('selections').length).to.equal(0);
    });
  });
